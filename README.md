...
# Search + mentions
bird search "from:erlinhoxha" -n 5
bird mentions -n 5
bird mentions --user @erlinhoxha -n 5

# User tweets (profile timeline)
bird user-tweets @erlinhoxha -n 20
bird user-tweets @erlinhoxha -n 50 --json
...
// Search for tweets
const searchResult = await client.search('from:erlinhoxha', 50);
...
```ts
const aboutResult = await client.getUserAboutAccount('erlinhoxha');
...
3. Browser cookies via Bird's built-in Safari/Chrome/Firefox readers (override via `--cookie-source` order)
