...
import type { TwitterClientBaseOptions } from './twitter-client-base.js';
...
const MixedTwitterClient: any = withNews(withUserTweets(withUserLookup(withUsers(withLists(withHome(withTimelines(withSearch(withTweetDetails(withPosting(withEngagement(withFollow(withBookmarks(withMedia(TwitterClientBase)))))))))) as any))));
