...
interface BrowserGetCookiesResult {
...
type CookieField = 'authToken' | 'ct0';
...
function minimalCookieHeader(authToken: string, ct0: string): string {
  return `auth_token=${authToken}; ct0=${ct0}`;
}

function cookieDomainRank(cookie: BrowserCookie): number {
...
function buildCookieHeaderFromCookies(
  cookies: BrowserCookie[],
  authToken: string,
  ct0: string,
): string {
  const bestByName = new Map<string, BrowserCookie>();
  ...
  const orderedNames = [...bestByName.keys()].sort((a, b) => {
    if (a === 'auth_token') return -1;
    if (b === 'auth_token') return 1;
    if (a === 'ct0') return -1;
    if (b === 'ct0') return 1;
...
function buildCookies(
  ...
  ct0: string,
  source: string,
  fullCookieHeader: string | null = null,
): TwitterCookies {
  return {
    authToken,
    ct0,
...
function readEnvCookie(cookies: TwitterCookies, keys: string[], field: CookieField): void {
  if (cookies[field]) {
    return;
  }
...
function isCachedCookiesRecord(value: unknown): value is CachedCookiesRecord {
  if (!value || typeof value !== 'object') {
    return false;
  }

  const record = value as Partial<CachedCookiesRecord>;
  return (
...
function loadCookieCache(): TwitterCookies | null {
  try {
    if (!existsSync(COOKIE_CACHE_FILE)) {
      return null;
    }

    const raw = JSON.parse(readFileSync(COOKIE_CACHE_FILE, 'utf8')) as unknown;
    ...
    return buildCookies(
      ...
      raw.ct0,
      raw.source ?? 'cache',
      typeof raw.cookieHeader === 'string' ? raw.cookieHeader : null,
    );
  } catch {
    return null;
  }
...
function saveCookieCache(cookies: TwitterCookies): void {
...
async function verifyCookies(cookies: TwitterCookies, userAgent: string): Promise<true> {
  if (!cookies.authToken || !cookies.ct0) {
    return true;
  }
...
function pickCookieValue(cookies: BrowserCookie[], name: string): string | null {
  const matches = cookies.filter(
    (cookie): cookie is BrowserCookie & { value: string } =>
      cookie?.name === name && typeof cookie.value === 'string',
  );
  if (matches.length === 0) {
    return null;
  }
...
async function readTwitterCookiesFromBrowser(
